...
func do_options_save() -> void:
	...
	CurrentAudioVolume = AudioScrollBar.value
	CurrentMusicVolume = MusicScrollBar.value
	CurrentEffectsVolume = EffectsScrollBar.value
...
func _on_reset_button_pressed() -> void:
	...
	dialog.confirmed.connect (func():
		...
		AudioScrollBar.value = CurrentAudioVolume
		MusicScrollBar.value = CurrentMusicVolume
		EffectsScrollBar.value = CurrentEffectsVolume
		do_options_save()
		dialog.queue_free()
	)
...
func _on_back_button_pressed() -> void:
	var changedDialog = func():
		...
		dialog.canceled.connect (func():
			AudioServer.set_bus_volume_db(MasterBus, linear_to_db(CurrentAudioVolume))
			AudioServer.set_bus_volume_db(MusicBus, linear_to_db(CurrentMusicVolume))
			AudioServer.set_bus_volume_db(EffectsBus, linear_to_db(CurrentEffectsVolume))
			save_complete.emit()
			dialog.queue_free()
		)
		add_child(dialog)
	...
	if LivesScrollBar.value != CurrentLives or ContinueScrollBar.value != CurrentContinues:
		changedDialog.call()
	elif AudioScrollBar.value != CurrentAudioVolume:
		changedDialog.call()
	elif MusicScrollBar.value != CurrentMusicVolume:
		changedDialog.call()
	elif EffectsScrollBar.value != CurrentEffectsVolume:
		changedDialog.call()
